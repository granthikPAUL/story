...
public class InvalidAdd extends Setup.Base {
	...
	String filePath3 = System.getProperty("user.dir") + "\\mainResources\\properties\\InvalidDataFile.properties";

	// Receiving the driver instance using consturctor
	public InvalidAdd(WebDriver driver) {
		InvalidAdd.driver = driver;
		prop = ReadProperties.readFile(filePath1);
		dataValid = ReadProperties.readFile(filePath3);
	...
	public void addRiskItem() throws IOException {
		try {
			...
			updateResult(indexSI++, "The New Risk has been created successfully for invalid data", "Pass",  "Test "+indexSI);
		} catch (Exception e) {
